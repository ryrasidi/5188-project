...
final_results <- purrr::map(final_results, ~ .x %>%
                             arrange(match(ID, c("1", "3", "6", "9", "12", "overall_RMSE"))))
summary_table <- Reduce(function(x, y) full_join(x, y, by = "ID"), final_results)
write(summary_table,"output.csv")
